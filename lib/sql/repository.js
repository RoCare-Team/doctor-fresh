...
/**
 * A build renders hundreds of pages; without memoisation each one would re-run
 * the same handful of queries. The catalogue is small (98 products, 18
 * categories, 36 posts), so the whole thing is held for a short TTL and shared
 * across renders.
 */
...
function memo(key, loader) {
  ...
  const value = loader().then(
    (v) => {
      // A failed load is not cached, so the next request retries the database.
      if (v === null) cache.delete(key);
      return v;
    },
    (err) => {
      cache.delete(key);
      console.error(`[repository] ${key} failed:`, err.message);
      return null;
    },
  );
...
export function getCategoriesFromDb() {
  ...
  return memo('categories', loadCategories);
}
...
export function getProductsFromDb() {
  if (!isDbEnabled()) return Promise.resolve(null);
  return memo('products', loadProducts);
}
...
export function getBlogsFromDb() {
  if (!isDbEnabled()) return Promise.resolve(null);
  return memo('blogs', loadBlogs);
}

/** Drops the memoised catalogue. */
