...
// The last good copy, kept for the case the database cannot answer at all —
// never to save a read, which would cost the tag. The same grace the rest of
// the catalogue gets.
const BLOG_GRACE_MS = Number(process.env.DB_CACHE_GRACE_MS) || 30 * 60_000;
let lastBlogs = null;
...
export function getBlogsFromDb() {
  ...
  return loadBlogsShared().then(
    ...
    () => (lastBlogs && Date.now() - lastBlogs.at < BLOG_GRACE_MS ? lastBlogs.value : null),
  );
}
