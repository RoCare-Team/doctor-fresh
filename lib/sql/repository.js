 ...
 * where a save can actually drop them: the framework's own store, which is
 * shared by every server, rather than one server's memory. The memory layer
 * above it stays, for the few seconds of a single render and for its last-good
 * copy when the database is unreachable — long enough to be useful, short
 * enough that a save is never baked into a page for the next five minutes.
 */
...
export function getBlogsFromDb() {
  if (!isDbEnabled()) return Promise.resolve(null);
  // null, so the memory layer falls back to its last good copy.
  return cached('blogs', () => loadBlogsShared().catch(() => null), 5_000);
}
