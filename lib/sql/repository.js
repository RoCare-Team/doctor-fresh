...
// the database cannot answer — the cache then serves the last good copy.
// Nothing here writes, alters or migrates.
...
// A collection may be served from its last good copy for this long while the
// database cannot answer — never to save a read, which would cost the tag the
...
export const PRODUCT_TAG = 'df-products';

const loadProductsShared = unstable_cache(
  async () => {
    const result = await loadProducts();
    if (result === null) throw new Error('products unavailable');
    return result;
  },
  ['df-products'],
  { tags: [PRODUCT_TAG], revalidate: 900 },
);
...
export function getProductsFromDb() {
  if (!isDbEnabled()) return Promise.resolve(null);
  return loadProductsShared().then(
    (value) => {
      lastProducts = { at: Date.now(), value };
      return value;
    },
    () => (lastProducts && Date.now() - lastProducts.at < GRACE_MS ? lastProducts.value : null),
  );
}
...
async function loadBlogs() {
  ...
  const catRows = await query(
    // Ordered so the category chips do not shuffle between renders.
    `SELECT \`blog_category_id\`, \`name\`, \`slug_url\`
       FROM \`${TABLES.blogCategories}\` ORDER BY \`blog_category_id\``,
  );
...
/**
 * Posts are the one collection edited in the admin all day, so they are held
 * where a save can actually drop them: the framework's own store, shared by
 * every server, under a tag.
 *
 * Every render of a blog page goes through this call, and that is the point —
 * a page that read the posts this way is filed under the same tag, so the
 * saved page is dropped along with the data. Put a plain memory cache in front
 * of it and the render never touches the tag, which is exactly how an edited
 * post could sit unchanged on the live site for five minutes after a save.
 */
export const BLOG_TAG = 'df-blogs';

const loadBlogsShared = unstable_cache(
  async () => {
    const result = await loadBlogs();
    // A failed read must not be stored, or every server would serve "no posts"
    // for the next five minutes; thrown, it leaves the store untouched.
    if (result === null) throw new Error('blogs unavailable');
    return result;
  },
  ['df-blogs'],
  { tags: [BLOG_TAG], revalidate: 900 },
);
...
export function getBlogsFromDb() {
  if (!isDbEnabled()) return Promise.resolve(null);
  return loadBlogsShared().then(
    (value) => {
      lastBlogs = { at: Date.now(), value };
      return value;
    },
    () => (lastBlogs && Date.now() - lastBlogs.at < GRACE_MS ? lastBlogs.value : null),
  );
}

/** Drops the cached catalogue, forcing the next read to hit the database. */
export { clearCache as clearRepositoryCache } from './cache';
