...
import { strip, delinkDomain, parseSeoSections, parseSpecTable, parseFaqs } from './html';
...
export function mapProduct(row, ctx = {}) {
  ...
  return {
    ...
    attributes,
    specifications: parseSpecTable(row.description),

    descriptionHtml: delinkDomain(row.description_new).trim(),
    installationHtml: delinkDomain(row.installation_commision).trim(),
    shippingHtml: delinkDomain(row.billing_shipping).trim(),
