...
import {
  ...
  withoutTables, firstWithProse, hasMedia, sanitizeRichHtml,
  tidyHtml,
} from './html';
...
/**
 * A meta description for the two catalogue rows that carry none, so no page
 * ships without one. It is written from the product's own copy rather than
 * invented: the opening of its description, and only if that is missing too,
 * its name and category.
 */
function productSummary(descriptionHtml, name, categoryName) {
  const prose = strip(descriptionHtml);

  if (prose.length >= 60) {
    if (prose.length <= 158) return prose;
    // Cut on a word so the sentence does not end mid-syllable.
    return `${prose.slice(0, 155).replace(/\s+\S*$/, '')}…`;
  }

  const what = categoryName ? `${name} — ${categoryName}` : name;
  return `${what} from Doctor Fresh. See the price, specification, features and warranty, with installation and service across India.`;
}
...
export function mapProduct(row, ctx = {}) {
  ...
  const descriptionHtml = delinkDomain(firstWithProse([
    row.description_new,
  ...
  return {
    ...
    metaDescription: strip(row.meta_description)
      || productSummary(descriptionHtml, name, category?.name),
    keywords: row.tag || '',
    ...
    // Unix seconds and a running counter, both kept by the PHP site.
    addedAt: num(row.add_timestamp),
    views: num(row.number_of_view),
  };
}
...
function seoFields(html, headingCol, introCol) {
  ...
  return {
    ...
    seoSections,
    // The full copy, only when it carries pictures or video — text-only copy
    // keeps the tidy section layout.
    richHtml: hasMedia(html) ? sanitizeRichHtml(html) : '',
  };
}
...
export function mapBlogPost(row, categorySlugs = []) {
  ...
  const content = tidyHtml(delinkDomain(row.description));
