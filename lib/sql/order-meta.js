...
// is_test: 1 = marked test, -1 = marked "not test" (overrides the test
// contacts rule), 0 = not marked either way.
export const EMPTY_META = {
  stage: 'active', isTest: false, notTest: false, courier: '', tracking: '', codCollected: false, remarks: [],
};
...
async function write(saleId, meta) {
  await mutate(
    ...
    [saleId, meta.stage, meta.isTest ? 1 : meta.notTest ? -1 : 0, meta.courier, meta.tracking, meta.codCollected ? 1 : 0, JSON.stringify(meta.remarks)],
  );
}
...
export async function updateOrderMeta(saleIds, patch, adminName = '') {
  ...
  for (const id of ids) {
    ...
    if (patch.isTest !== undefined) {
      meta.isTest = Boolean(patch.isTest);
      meta.notTest = !meta.isTest;
    }
