...
// Video hosts whose players may be embedded in page copy.
const EMBED_HOSTS = /^(?:https?:)?\/\/(?:www\.)?(?:youtube\.com|youtube-nocookie\.com|player\.vimeo\.com|google\.com\/maps|maps\.google\.com)\//i;

/** Does this copy carry pictures or video? */
export function hasMedia(html) {
  return /<(img|iframe|video)\b/i.test(String(html || ''));
}

/**
 * Page copy made safe to render as HTML: no scripts, styles, event handlers
 * or javascript: links, and iframes only from YouTube, Vimeo or Google Maps.
 * Everything else — headings, lists, tables, pictures, <video> — is kept, so
 * images and videos added in the admin show on the page.
 */
export function sanitizeRichHtml(html) {
  return delinkDomain(String(html || ''))
    .replace(/<(script|style|noscript|object|embed|form|input|button|textarea|select)\b[\s\S]*?(<\/\1>|\/>|$)/gi, '')
    .replace(/<!--[\s\S]*?-->/g, '')
    ...
    .replace(/<iframe\b[^>]*>[\s\S]*?<\/iframe>|<iframe\b[^>]*\/?>/gi, (tag) => {
      ...
      return `<div class="df-embed"><iframe src="${src}" loading="lazy" allowfullscreen `
        + 'allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" '
        + 'referrerpolicy="strict-origin-when-cross-origin" title="Embedded video"></iframe></div>';
    })
    .replace(/<img\b(?![^>]*\bloading=)/gi, '<img loading="lazy"')
    .replace(/<video\b(?![^>]*\bcontrols)/gi, '<video controls playsinline');
}

/**
 * Paste debris out of stored copy before it is shown: <style> blocks and
 * comments that came in with text pasted from a spreadsheet, and links with
 * nothing in them to see or tap (they are invisible, yet a finger lands on
 * them). A link holding a picture or any text is left as it is.
 */
export function tidyHtml(html) {
  return String(html || '')
    .replace(/<style\b[^>]*>[\s\S]*?<\/style>/gi, '')
    .replace(/<!--[\s\S]*?-->/g, '')
    .replace(/<a\b[^>]*>([\s\S]*?)<\/a>/gi, (link, inner) => (
      /<(img|video|iframe|svg|picture)\b/i.test(inner) || inner.replace(/<[^>]*>/g, '').replace(/&nbsp;|\s/g, '')
        ? link
        : inner
    ));
}
