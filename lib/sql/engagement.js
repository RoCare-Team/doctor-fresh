...
/**
 * Every address the PHP site may have stored a page's comments under: it saved
 * whatever host the visitor came in on (www., bare, mail.) and, for the older
 * service pages, the .php file name — /water-purifier-service.php and
 * /water-purifier-service are the same page.
 */
function commentUrls(url) {
  let path;
  try {
    path = new URL(url).pathname;
  } catch {
    return [clean(url, 500)];
  }
  path = path.replace(/\/+$/, '').replace(/\.php$/i, '') || '/';
  const hosts = ['https://www.doctorfresh.in', 'https://doctorfresh.in', 'https://mail.doctorfresh.in', 'http://www.doctorfresh.in', 'http://doctorfresh.in'];
  const out = new Set([clean(url, 500)]);
  for (const host of hosts) {
    out.add(`${host}${path}`);
    if (path !== '/') out.add(`${host}${path}.php`);
  }
  return [...out];
}

/** Comments are keyed by the page's absolute URL, as the PHP site stores them. */
export async function getComments(url) {
  const urls = commentUrls(url);
  const rows = await query(
    `SELECT \`id\`, \`name\`, \`comment\`, \`created_at\` FROM \`comment\`
      WHERE \`url\` IN (${urls.map(() => '?').join(',')}) AND \`status\` <> '0' ORDER BY \`id\` DESC`,
    urls,
  );
