...
/**
 * A build renders every page from one moment's data, so nothing needs to be
 * re-read part-way through. With a one-minute lifetime each build worker was
 * reloading the whole catalogue every minute — megabytes over a slow link, for
 * rows that had not changed — and pages timed out waiting behind it.
 */
const BUILDING = process.env.NEXT_PHASE === 'phase-production-build';
const TTL_MS = BUILDING ? Infinity : (Number(process.env.DB_CACHE_TTL_MS) || 60_000);
 ...
 */
export function cached(key, loader, ttlMs) {
  const entry = store.get(key);
  const ttl = BUILDING ? TTL_MS : (ttlMs ?? TTL_MS);
  if (entry?.fresh && Date.now() - entry.at < ttl) return entry.value;
