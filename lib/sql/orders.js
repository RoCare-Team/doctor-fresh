...
/** The GST registration from `business_settings`, exactly as stored. */
export async function getGstNumber() {
  const row = await queryOne(
    "SELECT `value` FROM `business_settings` WHERE `type` = 'gst_in_number' LIMIT 1",
  );
...
export async function getOrder({
  saleId, guestId, userId, asAdmin = false,
}) {
  const row = guestId
  ...
  // (The admin invoice route passes asAdmin, after its own permission check.)
  if (!guestId && !asAdmin && String(row.buyer) !== String(userId)) return null;
  ...
  return {
    ...
    // `sale_datetime` is the true unix time; `created_at` is on the DB's
    // clock, which runs hours behind India and can land on the previous day.
