...
import {
  strip, delinkDomain, dropPhp, parseSeoSections, parseFaqs, hasMedia, sanitizeRichHtml,
  tidyHtml,
} from './html';
...
// A build renders every page from one moment's data, and it warms this map a
// family at a time — one query for forty pages instead of forty. Serving reads
// from it is therefore right during a build and wrong afterwards, when a save
// has to be able to drop a page on every server at once.
...
function parseRelated(value) {
  ...
  return list
    .map((item) => {
      ...
      if (!name) return null;
      return { name, href: url ? dropPhp(delinkDomain(url)) : null };
    })
    .filter(Boolean);
}
...
/**
 * A description that says which page it belongs to.
 *
 * Fifty-four city pages — Agra, Ahmedabad, Amritsar and the rest — carry one
 * sentence written once for all of them, which leaves Google fifty-four pages
 * that describe themselves identically. Where the stored sentence never names
 * the city, the page's own service and city are put in front of it; the
 * sentence itself is left as written.
 */
function describes(text, serviceType, city) {
  const description = strip(text);
  if (!description || !city) return description;
  if (description.toLowerCase().includes(city.toLowerCase())) return description;

  // Some city names are stored in lower case ("baleshwar").
  const place = city.replace(/\b[a-z]/g, (ch) => ch.toUpperCase());
  const lead = serviceType ? `${serviceType} in ${place}` : place;
...
export function mapLandingPage(row) {
  ...
  const content = tidyHtml(delinkDomain(row.new_content?.trim() ? row.new_content : row.content));
  ...
  return {
    ...
    metaDescription: describes(row.meta_description, strip(row.service_type), strip(row.city)),
    keywords: row.meta_keywords || '',
    canonical: row.page_canonical ? dropPhp(delinkDomain(row.page_canonical)) : null,

    contentHtml: content,
    contentSections: sections,
    // Shown as written when the copy has pictures or video (see SeoContent).
    richHtml: hasMedia(content) ? sanitizeRichHtml(content) : '',
    // A YouTube / Vimeo link or an uploaded video file, set in the admin.
...
/** The tag one page is filed under, so a save can drop just that page. */
export const landingTag = (slug) => `df-landing:${slug}`;
...
export async function getLandingPageFromDb(slug) {
  ...
  if (BUILDING && cache.has(slug)) return cache.get(slug);
  // Past the build, only misses are held in memory (a bogus URL must not query
  // every time). A real page is held in the framework store instead, which
  // every server shares and an admin save can drop by tag — one server
  // clearing its own memory is what used to leave an edited page unchanged on
  // the live site.
  if (cache.get(slug) === null) return null;
  ...
  const read = unstable_cache(
    async () => {
      ...
      const page = mapLandingPage(row);
      // Nothing came back: either no such page or a database that could not
      // answer. Thrown rather than stored, so a bad minute cannot file a
      // real page away as missing for the next quarter of an hour.
      if (!page) throw new Error('no landing page');
      return page;
    },
    ['df-landing', slug],
    { tags: [landingTag(slug)], revalidate: 900 },
  );

  try {
    return await read();
  } catch {
    return remember(slug, null);
  }
}
...
export async function getLandingSlugsForBuild(perFamily = 40) {
  ...
  for (const family of families) {
    // A family of one is a national service page (AMC, installation…), linked
    // from the header — always built. City pages only as many as asked for.
    const count = Number(family.n) || 0;
    const take = count === 1 ? 1 : Math.min(count, perFamily);
    if (!take) continue;
...
/** Drops cached copies after an admin edit, so the next render reads the new row. */
export function forgetLandingPage(...slugs) {
  for (const slug of slugs) {
    if (!slug) continue;
    cache.delete(slug);
    // Reaches every server, and the stored page built from this row with it.
    try { revalidateTag(landingTag(slug)); } catch { /* not in a request */ }
  }
  nearbyCache.clear();
}
