...
export async function getLandingPageFromDb(slug) {
  ...
  // Only misses are held in memory (a bogus URL must not query every time).
  // A real page is held in the framework store instead, which every server
  // shares and an admin save can drop by tag — one server clearing its own
  // memory is what used to leave an edited page unchanged on the live site.
  ...
  const read = unstable_cache(
    async () => {
      ...
      // real page away as missing for the next five.
      if (!page) throw new Error('no landing page');
      return page;
    },
    ['df-landing', slug],
    { tags: [landingTag(slug)], revalidate: 300 },
  );
