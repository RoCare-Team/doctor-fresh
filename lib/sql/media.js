...
import { blobIndex, forgetBlobIndex } from '@/lib/blob';
...
/**
 * Drops the cached listing for an upload directory, so a photo added, replaced
 * or removed in the admin shows on the storefront without a server restart.
 */
export function forgetMedia(dir = UPLOAD_DIRS.product) {
  globalForMedia.__dfMedia?.delete(dir);
  forgetBlobIndex();
}

/** Every photo a product has, in the order the gallery should show them. */
export function productImages(id, declaredCount) {
  // Photos changed in the admin live in Vercel Blob and take precedence.
  const inBlob = blobIndex()?.product.get(Number(id));
  if (inBlob?.length) return inBlob.map((b) => b.url);
...
export function blogImage(id) {
  const inBlob = blobIndex()?.blog.get(Number(id));
  if (inBlob) return inBlob.url;
