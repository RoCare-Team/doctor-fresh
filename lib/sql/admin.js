...
export async function getDashboard(rangeId = DEFAULT_RANGE) {
  ...
  const [orders, allOrders, leads, callbacks, messages] = await Promise.all([
    // The DB clock runs on MST and `created_at` was written with different
    // offsets over the years, so "today" is the Indian day measured on the
    // unix `sale_datetime`. Sales count only what has actually been paid.
    query(
      `SELECT COUNT(*) n,
              SUM(CASE WHEN \`payment_status\` LIKE '%"paid"%' THEN \`grand_total\` ELSE 0 END) total
         FROM \`sale\` WHERE CAST(\`sale_datetime\` AS UNSIGNED) >= ?`,
      [from],
    ),
    // Every order ever placed, whatever window is being looked at.
    query(
      `SELECT COUNT(*) n,
              SUM(CASE WHEN \`payment_status\` LIKE '%"paid"%' THEN \`grand_total\` ELSE 0 END) total
         FROM \`sale\``,
  ...
  return {
    ...
    totalOrders: num(allOrders?.[0]?.n),
    totalSales: num(allOrders?.[0]?.total),
