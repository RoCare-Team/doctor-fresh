...
export async function getProduct(id) {
  ...
  return {
    ...
    descriptionHtml: row.description_new || '',
    shippingHtml: row.billing_shipping || '',
    installationHtml: row.installation_commision || '',
  };
}
...
export async function listBlogs({ limit = 200 } = {}) {
  ...
  const cats = await query('SELECT `blog_category_id`, `name` FROM `blog_category`');
...
export async function listBlogCategories() {
  const rows = await query('SELECT `blog_category_id`, `name`, `slug_url` FROM `blog_category`');
