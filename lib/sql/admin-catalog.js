...
export async function getProduct(id) {
  ...
  return {
    ...
    videoUrl: String(row.video || '').trim(),
    // The order matters — the product page shows the first six labels as its
    // highlights table — so it is kept exactly as stored.
    attributeValueIds: String(row.attribute_filters || '')
      .split(',')
      .map((v) => num(v))
      .filter(Boolean),
  };
...
/**
 * The labels a product's highlights are built from, each with the values it
 * offers — the same two tables the PHP panel fills (`attribute` and
 * `attribute_filter`), read here so the highlights can be picked from a list
 * rather than typed.
 */
/**
 * A new label (`attribute`). The PHP panel keeps a second, longer name for
 * its own lists; with nothing better to put there it gets the same name. The
 * sort number carries on from the last one, so the label lands at the end of
 * both panels rather than in the middle of an existing order.
 */
export async function createAttribute(title) {
  const name = clean(title, 200);
  if (!name) return { error: 'Enter a name.' };

  const existing = await queryOne(
    'SELECT `attribute_id`, `title` FROM `attribute` WHERE LOWER(`title`) = LOWER(?) LIMIT 1',
    [name],
  );
  if (existing) return { id: num(existing.attribute_id), title: clean(existing.title, 200), existed: true };

  const last = await queryOne('SELECT MAX(`sort_id`) AS `top` FROM `attribute`');
  const result = await mutate(
    'INSERT INTO `attribute` (`title`, `backend_title`, `sort_id`) VALUES (?, ?, ?)',
    [name, name, num(last?.top) + 1],
  );
  return { id: num(result?.insertId), title: name };
}

/** A new value under one label (`attribute_filter`). */
export async function createAttributeValue(attributeId, title) {
  const id = num(attributeId);
  const name = clean(title, 200);
  if (!id || !name) return { error: 'Enter a value.' };

  const label = await queryOne('SELECT `attribute_id` FROM `attribute` WHERE `attribute_id` = ? LIMIT 1', [id]);
  if (!label) return { error: 'That label no longer exists.' };

  const existing = await queryOne(
    'SELECT `attribute_filter_id`, `title` FROM `attribute_filter` WHERE `attribute_id` = ? AND LOWER(TRIM(`title`)) = LOWER(?) LIMIT 1',
    [id, name],
  );
  if (existing) {
    return {
      id: num(existing.attribute_filter_id),
      attributeId: id,
      title: clean(existing.title, 200),
      existed: true,
    };
  }

  const last = await queryOne('SELECT MAX(`sort_id`) AS `top` FROM `attribute_filter`');
  const result = await mutate(
    'INSERT INTO `attribute_filter` (`attribute_id`, `title`, `sort_id`) VALUES (?, ?, ?)',
    [id, name, num(last?.top) + 1],
  );
  return { id: num(result?.insertId), attributeId: id, title: name };
}

export async function listAttributes() {
  const [attributes, values] = await Promise.all([
    query('SELECT `attribute_id`, `title`, `sort_id` FROM `attribute` ORDER BY `sort_id`, `attribute_id`'),
    query('SELECT `attribute_filter_id`, `attribute_id`, `title`, `sort_id` FROM `attribute_filter` ORDER BY `sort_id`, `attribute_filter_id`'),
  ]);
  if (!attributes) return [];

  const byAttribute = new Map();
  for (const v of values || []) {
    const title = clean(v.title, 200);
    if (!title) continue;
    const key = num(v.attribute_id);
    if (!byAttribute.has(key)) byAttribute.set(key, []);
    byAttribute.get(key).push({ id: num(v.attribute_filter_id), title });
  }

  return attributes
    .map((a) => ({
      id: num(a.attribute_id),
      title: clean(a.title, 200),
      values: byAttribute.get(num(a.attribute_id)) || [],
    }))
    // A label with no values yet is kept: it was just created here, and its
    // first value is about to be typed into it.
    .filter((a) => a.title);
}
...
export async function updateProduct(id, fields) {
  ...
  if (fields.videoUrl !== undefined) put('video', clean(fields.videoUrl, 1000));
  ...
  // Stored the way the PHP panel stores them: value ids in one comma list, in
  // the order they are to appear.
  if (Array.isArray(fields.attributeValueIds)) {
    const ids = [...new Set(fields.attributeValueIds.map((v) => num(v)).filter(Boolean))].slice(0, 200);
    put('attribute_filters', ids.join(','));
  }
...
/**
 * Deletes a product row. Past orders are unaffected — each order keeps its own
 * copy of the name, price and quantity — and the photo files are left on disk
 * so those orders still show a picture. Pages that listed the product simply
 * skip an id that no longer exists.
 */
export async function deleteProduct(id) {
  const row = await queryOne(
    `SELECT p.\`product_id\`, p.\`slug\`, c.\`slug_url\` AS \`category_slug\`
       FROM \`${PRODUCTS}\` p LEFT JOIN \`category\` c ON c.\`category_id\` = p.\`category\`
      WHERE p.\`product_id\` = ? LIMIT 1`,
    [id],
  );
  if (!row) return { error: 'That product no longer exists.' };

  await mutate(`DELETE FROM \`${PRODUCTS}\` WHERE \`product_id\` = ?`, [id]);
  forgetAdminLists();
  return {
    ok: true,
    path: `/product/${row.slug}/${row.product_id}`,
    categoryPath: row.category_slug ? `/category/${row.category_slug}` : '/all-category',
  };
}
...
export async function updateBlog(id, fields) {
  ...
  if (fields.slug !== undefined) {
    ...
    if (!slug) return { error: 'Enter a URL for the post.' };

    // Only a *new* address has to be free. Some posts came over from the PHP
    // panel sharing an address with another, and refusing to save those would
    // mean their text could never be edited again.
    const current = await queryOne('SELECT `blog_url` FROM `blog` WHERE `blog_id` = ? LIMIT 1', [id]);
    if (slug !== String(current?.blog_url || '')) {
      const taken = await queryOne('SELECT `blog_id` FROM `blog` WHERE `blog_url` = ? AND `blog_id` <> ? LIMIT 1', [slug, id]);
      if (taken) return { error: `Another post already uses /${slug}.` };
    }
    put('blog_url', slug);
  }
...
/**
 * Off the site, or back on it. '0' is what the storefront treats as hidden
 * (REJECTED_STATUS); a live post carries an empty status, as the PHP panel
 * writes it.
 */
export async function setBlogLive(id, live) {
  await mutate('UPDATE `blog` SET `status` = ? WHERE `blog_id` = ?', [live ? '' : '0', num(id)]);
  return { ok: true };
}

/** The post itself; its cover picture and video are removed by the route. */
export async function deleteBlog(id) {
  const row = await queryOne('SELECT `blog_id` FROM `blog` WHERE `blog_id` = ? LIMIT 1', [num(id)]);
  if (!row) return { error: 'That post no longer exists.' };
  await mutate('DELETE FROM `blog` WHERE `blog_id` = ?', [num(id)]);
  return { ok: true };
}

/**
 * A new post, filled the way the PHP panel fills one (status left empty, which
 * is how every live post is stored; view count 0). Dated today unless given.
 */
export async function createBlog({
  title, slug, categoryId, author, date, excerpt, metaDescription,
}) {
  const cleanTitle = clean(title, 500);
  const cleanSlug = slugify(slug || cleanTitle);
  if (!cleanTitle) return { error: 'Enter a title.' };
  if (!cleanSlug) return { error: 'Enter a URL for the post.' };

  const taken = await queryOne('SELECT `blog_id` FROM `blog` WHERE `blog_url` = ? LIMIT 1', [cleanSlug]);
  if (taken) return { error: `Another post already uses /${cleanSlug}.` };

  const today = new Date().toLocaleDateString('en-CA', { timeZone: 'Asia/Kolkata' });
  const result = await mutate(
    `INSERT INTO \`blog\` (\`title\`, \`blog_url\`, \`summery\`, \`author\`, \`date\`, \`description\`, \`status\`,
                           \`blog_category\`, \`number_of_view\`, \`meta_description\`)
     VALUES (?, ?, ?, ?, ?, '', NULL, ?, 0, ?)`,
    [cleanTitle, cleanSlug, clean(excerpt, 1000), clean(author, 500), clean(date, 200) || today,
      categoryId ? String(num(categoryId)) : null, clean(metaDescription, 5000)],
  );
