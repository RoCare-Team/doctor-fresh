...
/**
 * Keeps the pooled connections open while the site is in use.
 *
 * A new connection to this host costs one to four seconds of handshake, and
 * both the pool (30s) and the server (60s) close a connection left idle — so
 * an admin who reads a page for half a minute used to pay that handshake on
 * the very next click. A one-row ping every 20s keeps the connections warm,
 * but only for ten minutes after the last real query: an idle site gives its
 * connections back to the shared account instead of holding them all night.
 */
const WARM_EVERY_MS = 20_000;
const WARM_FOR_MS = 10 * 60_000;

function keepWarm(pool) {
  globalForDb.__dfLastQuery = Date.now();
  if (BUILDING || globalForDb.__dfWarmTimer) return;

  const timer = setInterval(() => {
    if (Date.now() - (globalForDb.__dfLastQuery || 0) > WARM_FOR_MS) return;
    const warm = Math.min(2, POOL_SIZE);
    for (let i = 0; i < warm; i += 1) pool.query('SELECT 1').catch(() => {});
  }, WARM_EVERY_MS);
  // Never the reason a process stays alive.
  timer.unref?.();
  globalForDb.__dfWarmTimer = timer;
}
...
/**
 * One attempt, no retries — for work that sits in front of every page view
 * (the redirect lookup), where waiting ten seconds for a busy database is worse
 * than skipping the lookup this once.
 */
export async function queryOnce(sql, params = []) {
  const pool = getPool();
  if (!pool) return null;
  try {
    const [rows] = await pool.execute(sql, params);
    ...
    console.error('[db] single query failed:', err.code || err.message);
    return null;
  }
}
