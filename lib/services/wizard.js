...
export async function createBooking(fields) {
  ...
  // status 2 means that number already has an open visit with the service
  // team. Nothing is wrong with the request — the customer is already in the
  // queue — so it is reported as its own case rather than as a failure.
  if (data?.error && Number(data?.status) === 2) return { ok: false, duplicate: true };
