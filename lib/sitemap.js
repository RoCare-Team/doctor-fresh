...
const IS_LOCALITY = "(`page`.`locality` IS NOT NULL AND `page`.`locality` <> '' AND `page`.`locality` <> '0')";

/**
 * Only the row the site actually serves.
 *
 * 783 addresses have more than one row — the PHP panel allowed a slug to be
 * saved twice, usually under a different service type, which is how the same
 * address ended up listed in two sitemaps at once. The page itself serves the
 * lowest `page_id` of them, so that is the only row a sitemap should carry.
 *
 * Joined rather than matched row by row: the same rule written as a subquery
 * per row took minutes over this link, where the join takes 300 ms.
 */
const SERVED_ROWS = `\`page\`
       JOIN (SELECT \`page_url\`, MIN(\`page_id\`) AS \`served_id\`
               FROM \`${TABLES.landingPages}\`
              WHERE \`status\` = 'ok'
              GROUP BY \`page_url\`) AS \`served\`
         ON \`served\`.\`served_id\` = \`page\`.\`page_id\``;
...
export async function landingEntries(group) {
  const filters = ["`page`.`status` = 'ok'", '`page`.`service_type` <=> ?'];
  if (group.locality === true) filters.push(IS_LOCALITY);
  if (group.locality === false) filters.push(`NOT ${IS_LOCALITY}`);

  const rows = await query(
    `SELECT \`page\`.\`page_url\`, \`page\`.\`updated_at\`, \`page\`.\`created_at\`
       FROM \`${TABLES.landingPages}\` AS ${SERVED_ROWS}
...
async function otherLandingEntries() {
  ...
  const rows = await query(
    `SELECT \`page\`.\`page_url\`, \`page\`.\`updated_at\`, \`page\`.\`created_at\`
       FROM \`${TABLES.landingPages}\` AS ${SERVED_ROWS}
      WHERE \`page\`.\`status\` = 'ok'
        AND (\`page\`.\`service_type\` IS NULL
             OR \`page\`.\`service_type\` NOT IN (${known.map(() => '?').join(', ')}))
      ORDER BY \`page\`.\`page_id\` ASC`,
    known,
  );
