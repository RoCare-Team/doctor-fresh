...
/**
 * The signed-in admin with what they may do (`access`), or null. Access is
 * read fresh (cached ~30s), so switching someone off or changing their role
 * takes effect without waiting for their cookie to expire.
 */
export async function getAdminSession() {
  let admin;
  try {
    const store = await cookies();
    admin = parse(store.get(ADMIN_COOKIE)?.value);
  } catch {
    return null;
  }
